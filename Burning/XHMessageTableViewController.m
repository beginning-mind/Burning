...
- (void)finishRecorded {
    WEAKSELF
    NSLog(@"weakSelf.voiceRecordHelper.recordDuration: %f", weakSelf.voiceRecordHelper.currentTimeInterval);
    if (weakSelf.voiceRecordHelper.currentTimeInterval < 1.0) {
        [self cancelRecord];
        [self.voiceRecordHUD tooShortHint];
    }else {
...
- (void)didLongPressAvatorOnMessage:(id <XHMessageModel>)message atIndexPath:(NSIndexPath *)indexPath {
    ...
    [XHMessageTableViewController setBeAtUsername:curAvatarName];
    curAvatarName = [@"@" stringByAppendingString:curAvatarName];
    curAvatarName = [curAvatarName stringByAppendingString:@" "];
    
    [self.messageInputView longPressFromAvatar:curAvatarName];
}

-(void)didLongPressTextOnMessage:(id<XHMessageModel>)message atIndexPath:(NSIndexPath *)indexPath {
}


+(NSString*) getBeAtUsername {
    return beAtUsername;
}

+(void) setBeAtUsername:(NSString*)username {
