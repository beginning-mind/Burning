...
- (UIButton *)deleleBtn{
    if (!_deleleBtn) {
        ...
        [deleleBtn setImage:[UIImage imageNamed:@"AssetsPickerChecked"] forState:UIControlStateNormal];
...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *barButtonItemLeft=[[UIBarButtonItem alloc] initWithTitle:@"返回" style:UIBarButtonItemStyleBordered target:self action:@selector(back)];
...
- (void)deleteAsset{
    ...
    if ([_deleteAssets valueForKeyPath:currentPage] == nil) {
        ...
        [self.deleleBtn setImage:[[UIImage imageNamed:@"AssetsPickerChecked" ] imageWithTintColor:[UIColor grayColor]] forState:UIControlStateNormal];
        ...
        if ([self.doneAssets containsObject:[self.photos objectAtIndex:self.currentPage]]) {
            ...
            [self.selectsIndexPath removeObjectAtIndex:self.currentPage];
        }
    }else{
        if (![self.doneAssets containsObject:[self.photos objectAtIndex:self.currentPage]]) {
        ...
        [self.deleleBtn setImage:[UIImage imageNamed:@"AssetsPickerChecked" ] forState:UIControlStateNormal];
    }
