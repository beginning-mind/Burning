...
//        NSLog(@"curDate1:%@",[formatter stringFromDate:curDate1]);
//        
//        AVPlayerItem *playerItem = [AVPlayerItem playerItemWithURL:url];
//        
//        NSDate *curDate2 = [NSDate date];
//        NSLog(@"curDate2:%@",[formatter stringFromDate:curDate2]);
//        
//        totalTime.value += playerItem.asset.duration.value;
//        
//        NSDate *curDate3 = [NSDate date];
//        NSLog(@"curDate3:%@",[formatter stringFromDate:curDate3]);
//        
...
//            UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"ss" message:@"ss" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
//            [alertView show];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (_gestureType == GestureTypeOfNone) {
        if ((currentLocation.x > frame.size.width*0.8) && (ABS(offset_x) <= ABS(offset_y))){
            _gestureType = GestureTypeOfVolume;
        }
        else if ((currentLocation.x < frame.size.width*0.2) && (ABS(offset_x) <= ABS(offset_y))){
    ...
    if (_gestureType == GestureTypeOfNone && !CGRectContainsPoint(_bottomView.frame, point) && !CGRectContainsPoint(_topView.frame, point)) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            if (topFrame.origin.y<0) {
                //显示
                topFrame.origin.y = 0;
                bottomFrame.origin.y = self.view.bounds.size.height-BottomViewHeight;
                [self performSelector:@selector(hidenControlBar) withObject:nil afterDelay:3];
            }else{
                //隐藏
