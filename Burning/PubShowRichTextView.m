...
-(UIButton*)deleteBtn{
    if (_deleteBtn==nil) {
        ...
        [_deleteBtn setTitleColor:RGB(199, 199, 199) forState:UIControlStateNormal];
...
//    _shareCollectionView.frame=CGRectMake(0,CGRectGetMaxY(self.avatarImageView.frame)+kSmallTextVerticSpace, [SVGloble shareInstance].globleWidth, [[self class] getCollectionViewHeightWithPhotoCount:_publish.publishPhotos.count]);
    ...
    _deleteBtn.frame = CGRectMake([SVGloble shareInstance].globleWidth-kPubShowCommentButtonWidth-kCommonAvatarLeadingSpace, CGRectGetMinY(self.likesButton.frame), kPubShowCommentButtonWidth, kPubShowCommentButtonHeight);

    if ([[AVUser currentUser].objectId isEqualToString:self.publish.creator.objectId]) {
...
-(void)deleteBtn:(UIButton*)sender{
    UIAlertView *alterView = [[UIAlertView alloc]initWithTitle:nil message:@"确定删除" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
    [alterView show];
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if (buttonIndex ==1) {
        //delete publish
        NSError *err;
        [self.publish setObject:[NSNumber numberWithInt:1] forKey:KEY_IS_DEL];
        [self.publish saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
            NSString *deleteInfo;
            if (succeeded) {
                deleteInfo = @"删除成功";
                if ([_richTextViewDelegate respondsToSelector:@selector(didDeletePubButtonClick)]) {
                    [_richTextViewDelegate didDeletePubButtonClick];
                }
            }
            else{
                deleteInfo = @"删除失败";
            ...
            [AFMInfoBanner showAndHideWithText:deleteInfo style:AFMInfoBannerStyleInfo];
