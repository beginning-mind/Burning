...
- (void)addDailyNewsObserver:(id)target selector:(SEL)selector{
    [_center addObserver:target selector:selector name:NOTIFICATION_DAILYNEWS_UPDATED object:nil];
}

- (void)removeDailyNewsObserver:(id)target{
    [_center removeObserver:target name:NOTIFICATION_DAILYNEWS_UPDATED object:nil];
}

-(void)postDailyNewsNotify{
    [_center postNotificationName:NOTIFICATION_DAILYNEWS_UPDATED object:nil];
}
