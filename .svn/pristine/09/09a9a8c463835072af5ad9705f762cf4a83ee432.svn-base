...
//        imageView.layer.cornerRadius = kCommonAvatarSize/2.0;
        ...
        if (z==avatarCount && _lcActivity.attendUsers.count>avatarCount) {
            ...
            [imageView addGestureRecognizer:singleTap];
            if (z==0) {
                UIImageView *tagImageView = [[UIImageView alloc]initWithFrame:CGRectMake(CGRectGetMaxX(imageView.frame)-9, CGRectGetMaxY(imageView.frame)-8, 14, 10)];
...
-(void)loadCommentView{
    ...
    for (LCActivityComment *comment in _lcActivity.comments) {
        ...
        CGFloat height=[NSString getTextHeight:content textFont:[UIFont systemFontOfSize:kSmallFontSize] width:CGRectGetWidth([[UIScreen mainScreen] bounds])-2*kCommonAvatarLeadingSpace-2*kSmallSpace];
...
-(void)layoutSubviews{
    ...
    _placeLabel.frame = CGRectMake(CGRectGetMaxX(_place.frame)+kCommonSpace, CGRectGetMinY(_place.frame),[[self class] contentWidth], [NSString getTextHeight:_placeLabel.text textFont:[UIFont systemFontOfSize:kCommonFontSize] width:[[self class] contentWidth]]);
    ...
    _contentLabel.frame = CGRectMake(kCommonSpace, CGRectGetMaxY(_content.frame)+kCommonSpace, CGRectGetWidth([[UIScreen mainScreen] bounds])-2*kCommonAvatarLeadingSpace,  [NSString getTextHeight:_contentLabel.text textFont:[UIFont systemFontOfSize:kSmallFontSize] width:[[self class] contentWidth]]);
...
-(void)moreUserClick:(UIGestureRecognizer*)gestureRecognizer{
    ...
    userListVC.users = self.lcActivity.attendUsers;
    BaseViewController *baseVC = [self getSuperViewController];
    [baseVC.navigationController pushViewController:userListVC animated:YES];
}
