...
- (void)setSelectPickerAssets:(NSArray *)selectPickerAssets{
    ...
    for (MLSelectPhotoAssets *assets in selectPickerAssets) {
        if ([assets isKindOfClass:[MLSelectPhotoAssets class]]) {
            [self.collectionView.selectAsstes addObject:assets];
        }
    }
...
//    self.collectionView.selectAsstes = self.selectAssets;
    NSInteger count = self.collectionView.selectAsstes.count;
...
//    self.makeView.text = [NSString stringWithFormat:@"%ld",(long)count];
    self.doneBtn.enabled = (count > 0);
    self.previewBtn.enabled = (count > 0);
//     self.makeView.text = [NSString stringWithFormat:@"%ld",self.collectionView.selectAsstes.count];
...
- (void) setupButtons{
    ...
    UIBarButtonItem *backItem = [[UIBarButtonItem alloc]initWithTitle:@"选择相册" style:UIBarButtonItemStylePlain target:self action:@selector(back:)];
    self.navigationItem.leftBarButtonItem = backItem;
    backItem.title = @"选择相册";
    
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"na_close"] style:UIBarButtonItemStylePlain target:self action:@selector(cancel)];
}
...
//    toorBar.barTintColor = [UIColor blackColor];
...
-(void)setMinCount:(NSInteger)minCount{
    ...
    if (self.collectionView.selectAsstes.count == minCount){
        minCount = 0;
    }else if (self.selectPickerAssets.count - self.collectionView.selectAsstes.count > 0) {
...
//    self.makeView.text = [NSString stringWithFormat:@"%ld",(long)count];
    ...
    if (self.selectPickerAssets.count || deleteAssets) {
        ...
        for (NSInteger i = 0; i < self.collectionView.selectAsstes.count; i++) {
            MLSelectPhotoAssets *photoAsset = self.collectionView.selectAsstes[i];
...
//        self.makeView.text = [NSString stringWithFormat:@"%ld",self.collectionView.selectAsstes.count];
        ...
        // 刷新下最小的页数
        self.minCount = self.collectionView.selectAsstes.count + (_privateTempMinCount - self.collectionView.selectAsstes.count);
    }
}
...
- (NSInteger) collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    return self.collectionView.selectAsstes.count;
}
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.collectionView.selectAsstes.count > indexPath.item) {
        ...
        imageView.image = [self.collectionView.selectAsstes[indexPath.item] thumbImage];
    }
...
- (void)dealloc{
    // 赋值给上一个控制器
    self.groupVc.selectAsstes = self.collectionView.selectAsstes;
}
