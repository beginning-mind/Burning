...
-(void)getPublishsWithUserID:(NSString*)userID limit:(NSInteger*)limit skip:(NSInteger*)skip block:(AVArrayResultBlock)block{
    @try {
        AVQuery *avq = [AVUser query];
        [avq whereKey:@"objectId" equalTo:userID];
        [avq setCachePolicy:kAVCachePolicyNetworkElseCache];
        [avq findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
            if (error) {
                ...
                if (objects>0) {
                    ...
                    [q includeKey:@"comments.toUser"];
                    [q whereKey:KEY_IS_DEL equalTo:@0];
                    [q whereKey:KEY_CREATOR equalTo:curUser];
                    [q setCachePolicy:kAVCachePolicyNetworkElseCache];
                    [q findObjectsInBackgroundWithBlock:block];
                }
            }
...
-(void)publishWithContent:(NSString*)content images:(NSArray*)images error:(NSError**)error{
    ...
    for (UIImage *image in images) {
        ...
        //NSLog(@"[imageData length]:%d", [originData length]);
        NSData *imageData;
        if (len < 500*1024 || len == 500*1024) {
            imageData = UIImageJPEGRepresentation(image, 0.8);
        }else if(len > 500*1024 && len <1000*1024){
